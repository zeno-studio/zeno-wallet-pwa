<script lang="ts">
	...
	import { handleSetTimer, handleAutoLock ,type signerResponseType} from '$lib/wallet/runes';
	...
	updatePageTitle(1, 'Auto-lock timer (minutes)');

	let isDone = $state<boolean | null>(null);

	const handleSetAutoLock = async (time: number) => {
        if(time>0){
            const result = (await handleSetTimer(time)) as signerResponseType | null;
		if (result?.success) {
			isDone = true
		} 
    }
        if(time===0){
            const result = (await handleAutoLock (false)) as signerResponseType | null;
		if (result?.success) {
			isDone = true
		} 
    }
